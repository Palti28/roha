...
(() => {
  ...
  function mount() {
    ...
    // Internal listeners (bound once at mount)
  ...
  // ---------- Resolve data on a trigger ----------
  function readData(trigger) {
    // Look on trigger itself; fall back to closest ancestor that carries the url
  ...
  function open(trigger) {
    ...
    e.url.textContent   = data.title;            // pill shows the product name

    // Open-in-new-tab button — only show when there's a real URL
    ...
    // Load iframe (or trigger fallback if missing)
    clearTimeout(fallbackTimer);
    if (data.url && data.url !== '#') {
  ...
  function setViewport(vp) {
    ...
    // Custom aspect ratio only overrides the desktop preset
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!trigger) return;
    // Modifier / middle-click passes through to native link behavior
  ...
  // Keyboard activation for non-button/non-anchor triggers (e.g. <span role="button">)
  document.addEventListener('keydown', (e) => {
    ...
    if (tag === 'BUTTON' || tag === 'A') return; // native already handles these
    e.preventDefault();
    open(trigger);
  });
})();
